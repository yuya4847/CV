...
この PJ では、上記をメインに新機能の追加実装が主な担当業務だった。フロントエンドとバックエンド両方を担当した(フロントエンドがメイン)。アトミックデザインを用いた開発を経験することができた。主に、新規登録画面のページ作成と行い、ログアウトやログイン画面の作成を担当した。また、リリース前にスタイル・バグ修正を行うことでプロダクトの品質向上に貢献した。
